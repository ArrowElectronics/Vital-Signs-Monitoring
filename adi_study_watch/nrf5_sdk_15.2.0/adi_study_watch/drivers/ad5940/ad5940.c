...
* @version
* @date         26-Jun-2019
* @brief        Source file for system functions related to AD5940
*
***************************************************************************
* @attention
***************************************************************************
 ...
 * @param event
 */
void ad5940_spi_event_handler(nrf_drv_spi_evt_t const * p_event,
                       void *                    p_context)
{
    if(p_event->type == NRF_DRV_SPI_EVENT_DONE)
    {
      ad5940_spi_xfer_done = true;
      //ad5940_spi_uninit(); /*commenting this line; as it is being called after every read/write operations*/
      //adi_osal_SemPost(ad5940_spi_txfr_sem);
    }
}

void ad5940_spi_thread_init()
{
	//mutex create , sem create
	adi_osal_MutexCreate(&ad5940_spi_mutexlock);
  	//adi_osal_SemCreate(&ad5940_spi_txfr_sem, 0U);
}
...
static void ad5940_spi_init(void)
{
    ...
    spi_config.frequency = NRF_DRV_SPI_FREQ_8M;//can support 16M,after the PCB arrived to test.
    //mutex lock acquire
    adi_osal_MutexPend(ad5940_spi_mutexlock, ADI_OSAL_TIMEOUT_FOREVER);
    nrf_drv_spi_init(&ad5940_spi_handler, &spi_config, ad5940_spi_event_handler, NULL);
    nrf_gpio_cfg_output(AD5940_SPI_CS_PIN);
    nrf_gpio_pin_set(AD5940_SPI_CS_PIN);//

}

static void ad5940_spi_uninit(void)
{
    nrf_drv_spi_uninit(&ad5940_spi_handler);
    //mutex lock released
    adi_osal_MutexPost(ad5940_spi_mutexlock);
}

volatile static uint32_t ucInterrupted = 0;       /* Flag to indicate interrupt occurred */
static uint32_t InterruptCount = 0;
static void ad5940_int0_event_handler(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
    //NRF_LOG_INFO("ad5940_int0 low");
    ucInterrupted = 1;
    InterruptCount++;
    NRF_LOG_DEBUG("Interrupt count =%d", InterruptCount);

    if (gpfnAD5940CallBack != NULL) {
        (*gpfnAD5940CallBack)();
    }
}
static uint32_t ad5940_int0_init(void)
{
    ...
    //nrf_drv_gpiote_out_init(AD5940_RESET_PIN, &out_config);

    err_code = nrf_drv_gpiote_in_init(AD5940_INT0_PIN, &in_config, ad5940_int0_event_handler);
    VERIFY_SUCCESS(err_code);
    nrf_drv_gpiote_in_event_enable(AD5940_INT0_PIN, true);
...
static uint32_t ad5940_int0_deinit(void)
{
    ...
    nrfx_gpiote_in_event_disable(AD5940_INT0_PIN);
    nrfx_gpiote_in_uninit(AD5940_INT0_PIN);
...
{
    nrf_gpio_pin_clear(AD5940_SPI_CS_PIN);//
}
...
#if 0
__STATIC_INLINE ret_code_t AD5940_SPI_Transmit(uint8_t *data, uint8_t length)
{
  ADI_HAL_STATUS_t nStatus = ADI_HAL_OK;
  ret_code_t ret;
  //adi_osal_MutexPend(ad5940_spi_mutexlock, ADI_OSAL_TIMEOUT_FOREVER);
  ret = nrf_drv_spi_transfer(&ad5940_spi_handler, data, length, NULL, 0);
  nStatus = (!ret) ? ADI_HAL_ERROR : ADI_HAL_OK;
  while(!ad5940_spi_xfer_done);
  ad5940_spi_xfer_done = false;
  //adi_osal_SemPend(ad5940_spi_txfr_sem, ADI_OSAL_TIMEOUT_FOREVER);
  //adi_osal_MutexPost(ad5940_spi_mutexlock);
  return (nStatus);
//    return (nrf_drv_spi_transfer(&ad5940_spi_handler, data, length, NULL, 0));
}
#endif

__STATIC_INLINE ret_code_t AD5940_SPI_Receive(uint8_t *data_tx,uint8_t *data_rx,uint8_t length)
{

  ADI_HAL_STATUS_t nStatus = ADI_HAL_OK;
  ret_code_t ret;
  //adi_osal_MutexPend(ad5940_spi_mutexlock, ADI_OSAL_TIMEOUT_FOREVER);
  ret = nrf_drv_spi_transfer(&ad5940_spi_handler, data_tx, length, data_rx, length);
  nStatus = (!ret) ? ADI_HAL_ERROR : ADI_HAL_OK;
  while(!ad5940_spi_xfer_done);
  ad5940_spi_xfer_done = false;
  //adi_osal_SemPend(ad5940_spi_txfr_sem, ADI_OSAL_TIMEOUT_FOREVER);
...
__STATIC_INLINE void MCU_HAL_Delay(uint32_t length)
{
   vTaskDelay(length);
}
...
void AD5940_FIFORd(int32_t *pBuffer, uint32_t uiReadCount)
{
  while(uiReadCount--)
 ...
 * @brief The basic SPI protocols. All functions are basic on AD5940_SPI_Receive which
 *        provided by user.
 *
 *  ##SPI basic protocol
...
static uint16_t AD5940_ReadWrite16B(uint16_t data)
{
   ...
   AD5940_SPI_Receive(SendBuffer,RecvBuffer,2);
...
static uint32_t AD5940_ReadWrite32B(uint32_t data)
{
   ...
   AD5940_SPI_Receive(SendBuffer,RecvBuffer,4);
...
/** @brief  Register data ready callback
  *
  * @param  pfAD5940DataReady  Function Pointer callback for the reg data
  * @return None
  */
void Ad5940DrvDataReadyCallback(void (*pfAD5940DataReady)()) {
    gpfnAD5940CallBack = pfAD5940DataReady;
}

/** @brief  AD5940 interrupt service routine
  * @param  None
  * @return None
  */
