...
struct m2m2_sensor_adpd4000_data_stream_t {
  uint8_t  command; 
  uint8_t  status; 
  uint16_t  sequence_num; 
  uint16_t  data_format; 
...
struct m2m2_adpd_dcfg_op_hdr_t {
  ...
  m2m2_adpd_dcfg_op_t  ops[1]; // NOTE: THIS FIELD IS INTENDED TO BE OF VARIABLE LENGTH! 
        // NOTE: Use offsetof(m2m2_adpd_dcfg_op_hdr_t, ops) instead of sizeof(m2m2_adpd_dcfg_op_hdr_t)
};
